...
> **状态**: v3（增加 Scheduler Docker 生命周期管理 & 可靠性设计）
> **更新说明**: 增加 round 脚本参数化修改为第一优先任务；补充完整的 config_data schema；修正 SSE 实现；新增 Scheduler Docker 生命周期管理（stop_services/reconcile_services）；新增可靠性设计（超时/重试/心跳）；更新实施总览图新增 [1.4][1.5]
...
│   ├── [1.3] Pipeline Scheduler 骨架（asyncio 主循环 + executor）
│   ├── [1.4] Docker 生命周期管理（stop_services + reconcile_services）
│   └── [1.5] docker-compose.yml 移除 restart: unless-stopped
│
├── Milestone 2：认证
...
│   ├── [3.2] Scheduler 集成（round 调用 + 超时/重试/心跳）
│   ├── [3.3] 前端任务列表页 + 详情页
...
**文件**：
- 创建：`platform/scheduler/main.py`（主循环 + 崩溃恢复）
- 创建：`platform/scheduler/executor.py`（subprocess 管理 + 超时 + 重试）
- 创建：`platform/scheduler/db.py`（PostgreSQL 查询封装）
...
async def scheduler_loop():
    while True:
        # 1. 崩溃恢复：检查 running job 的心跳
        for job in select_running_jobs():
            if is_stale(job):  # last_heartbeat > 5min
                mark_failed(job.id, "Scheduler 重启")
            else:
                continue_from_checkpoint(job)

        # 2. 取下一个 pending job（原子操作）
        job = select_next_pending_job()  # FOR UPDATE SKIP LOCKED
        if not job:
...
**executor.py — round 执行器**：

```python
async def execute_job(job):
    for round_name in ROUND_SEQUENCE:
        # Docker 生命周期协调
        reconcile_services([{"current_round": round_name}])

        # 注入环境变量
        env = os.environ.copy()
        env["PIPELINE_JOB_CONFIG"] = json.dumps(config)
        env["SKIP_DOCKER_START"] = "1"  # round 脚本不再管理容器
        ...
        stdout, stderr = await asyncio.wait_for(
            proc.communicate(), timeout=ROUND_TIMEOUT  # 6h
        )

        if proc.returncode != 0:
            mark_failed(job_id, stderr[-500:])
            return

    reconcile_services([])  # 清理所有容器
    update_status(job_id, "completed")
```

### [1.4] Docker 生命周期管理（新增）

在 `s4_docker_utils.py` 中新增：

- **`stop_services(services)`**：停止并移除指定容器，清除 health cache
- **`reconcile_services(active_jobs)`**：对比"全局需要"与"实际运行"，自动启停差异

所有 round 脚本增加 `SKIP_DOCKER_START` 环境变量检测，round 内部不再管理容器启动。

### [1.5] docker-compose.yml 移除 restart: unless-stopped（新增）

将 `tools/docker-compose.yml` 中所有微服务的 `restart: unless-stopped` 改为 `restart: "no"`。容器生命周期由 Scheduler 全权管理。
...
### [3.2] Scheduler 集成（带参数调用 round + 可靠性）

**必须完成的改动**（接 Milestone 0 & 1.3-1.5）：

**Round 调用**：
1. Scheduler 读取 `jobs.config_snapshot` JSON，提取各 round 参数
2. 通过 `PIPELINE_JOB_CONFIG` 环境变量注入（JSON 可能超过 CLI 长度限制）
3. 设置 `SKIP_DOCKER_START=1` 环境变量，round 脚本跳过容器管理
4. 每 round 前调用 `reconcile_services([job])` 协调容器
5. 每 round 完成后写入 `job_rounds` + `job_events`（直接 SQL）
6. Round 出错时写入 `job_events(event='job_failed')` 并更新 `jobs.status='failed'`
7. 全部 round 完成后调用 `reconcile_services([])` 清理所有容器

**可靠性**：
8. 每 round 启动 `asyncio.wait_for(proc.communicate(), timeout=6h)` 硬超时保护
9. 可重试 round（HTTP 调用类）失败后指数退避重试（2 次，5s→15s）
10. 不可重试 round（GPU 长任务、本地计算）失败直接记为 failed
11. Scheduler 每隔 60s 更新 `jobs.last_heartbeat` 心跳
...
| Round 脚本 hang 不退出 | `asyncio.wait_for(..., timeout=6h)` 硬超时 kill subprocess |
| Scheduler 崩溃后恢复 | 崩溃恢复逻辑 + `last_heartbeat` 检测（超过 5 分钟标记 failed）|
| Docker 容器停止后 GPU 显存不释放 | `docker compose rm -f -s`（SIGTERM + 移除）确保 CUDA 上下文完全清理 |
| Scheduler 与 round 脚本双重管理容器 | `SKIP_DOCKER_START=1` 环境变量让 round 跳过容器管理 |