#!/usr/bin/env python3
"""导出 Top 10 + Bottom 10 (对照组) 到 CSV，附带完整 10 项分数 + 4 种递送综合分。

用法：
    python3 scripts/pipeline/export_antibacterial_top10.py
输出：
    results/pipeline/antibacterial_v1/top10.csv
    results/pipeline/antibacterial_v1/bottom10.csv
    results/pipeline/antibacterial_v1/top10_bottom10.csv
"""
...
def main() -> int:
    out_dir = ROOT / "results" / "pipeline" / "antibacterial_v1"
    out_dir.mkdir(parents=True, exist_ok=True)

    # 三个输出：纯 Top10、纯 Bottom10、Top10+Bottom10 综合版
    queries = [
        ("top10.csv",
         "WHERE c.direction = 'antibacterial' AND c.channel = 'top' AND c.rank <= 10\n"
         "ORDER BY c.rank",
         False),
        ("bottom10.csv",
         "WHERE c.direction = 'antibacterial' AND c.channel = 'bottom'\n"
         "  AND c.peptide_id IN (\n"
         "    SELECT peptide_id FROM constructs\n"
         "     WHERE direction = 'antibacterial' AND channel = 'bottom' AND status = 'passed'\n"
         "     ORDER BY (scores->>'amp_esm')::float8 ASC LIMIT 10)\n"
         "ORDER BY (c.scores->>'amp_esm')::float8 ASC",
         True),
        ("top10_bottom10.csv",
         "WHERE c.direction = 'antibacterial'\n"
         "  AND ( (c.channel = 'top'    AND c.rank <= 10)\n"
         "     OR (c.channel = 'bottom' AND c.peptide_id IN (\n"
         "           SELECT peptide_id FROM constructs\n"
         "            WHERE direction = 'antibacterial' AND channel = 'bottom' AND status = 'passed'\n"
         "            ORDER BY (scores->>'amp_esm')::float8 ASC LIMIT 10) ) )\n"
         "ORDER BY c.channel DESC, c.rank NULLS LAST, (c.scores->>'amp_esm')::float8 ASC",
         True),
    ]
