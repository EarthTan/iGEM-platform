...
三步：
  1. 在 Top 通道上算分量（plm4cpps / algpred2 / bepipred / aggrescan_a3v）：
        * 归一化到 [0,1]
        * 方向统一为"越高越好"
...
from . import DELIVERY, AntioxidantConfig, aggrescan_a3v, to_component, winsorized_std

log = logging.getLogger("antioxidant.scoring")
...
def compute_components(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.Series], dict[str, float]]:
    ...
    comps = {
        ...
        "epitope":  to_component(df["bepipred"],      False) if "bepipred" in df.columns else pd.Series(0.5, index=df.index),
        "agg":      to_component(df["aggrescan_a3v"], False),   # 越高越促聚集 → 翻转
    }

    # 仅有 NaN 的分量（如 bepipred 全 NULL）给 0 区分度 → 权重自动归零
    ...
    comps_df = pd.concat(comps, axis=1)        # 列名变成 ('cpp', 0) 这种 MultiIndex
...
def composite(
    ...
    # 注射下免疫阈值收紧：这里直接对 mhcflurry 高的肽判 failed_safety → 0 分
    if delivery == "injection" and "mhcflurry" in df.columns:
        fail_mask = df["mhcflurry"].notna() & (df["mhcflurry"] >= cfg.mhci_threshold_injection)
    else:
...
def anoxpepred_confirm(df: pd.DataFrame) -> pd.Series:
    """AnOxPePred FRS 辅助确认：≥ 0.5 标记 1（仅 500K 内有值的少数肽会触发）。"""
    if "anoxpepred_frs" not in df.columns:
        return pd.Series(0, index=df.index, dtype="int8")
    return (df["anoxpepred_frs"].fillna(0) >= 0.5).astype("int8")